...
const formData = reactive({
    id: SettingRepository.serviceGroup.id,
...
const save = async () => {
    ...
    if (isValid) {
        if (SettingRepository.isEditMode) {
            await SettingRepository.UpdateServiceGroup(formData.id, formData);
        } else {
            await SettingRepository.CreateServiceGroup(formData);
        }
    }
};
</script>
